...
var pName_Array = [];
var hasCreateInputModalRun = false;
        ...
        for(let i = 0; i<number_of_players; i++){
            ...
            if(modal_box_var_gen_array.includes(modal_box_var_gen_array[j])){
                ...
                modal_skip_button_var_gen_array[modal_skip_button_var_gen_array.length-1].onclick = function(){
                    ...
                    slide_player_input_modal_box_left(input_field_divs_var_gen_array[i])
                    all_players_modal_background.style.display = 'none';

                    hasCreateInputModalRun = true;

                    // SET PLAYER NAMES TEXT CONTENT
                    setPlayerNamesTextContent();
...
function decrementNumberofPlayers(){
    number_of_players --;
}

function setPlayerNamesTextContent(){
    playerElements.p1Button.textContent = pName_Array[0];
    playerElements.p2Button.textContent = pName_Array[1];
}
